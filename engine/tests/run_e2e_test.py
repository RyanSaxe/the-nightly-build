...
subprocess.run(f"git archive main | tar -x -C {maindir}",
...
def library_state():
    ...
    subprocess.run(
        f"git archive library | tar -x -C {libdir}",
        shell=True, cwd=root, check=True)
    return libdir
...
def night_shift_run(branch, series, slug, html, today, from_ref="library"):
    ...
    meta = json.loads(
        html.split('id="nb-meta">')[1].split("</script>")[0])
    body.write_text(
        "Nightly edition.\n\n```nb-meta\n"
        f"series: {series}\nslug: {slug}\nmode: {meta['mode']}\n"
    ...
    proof = run([sys.executable, str(pathlib.Path(maindir, "engine", "check.py")),
                 "--pr", "--repo", root, "--main", maindir,
                 "--base", "library", "--head", branch,
                 "--library", library_state(), "--pr-body", str(body),
                 "--today", today, "--json"], cwd=root)
    findings = json.loads(proof.stdout)
    auto = run([sys.executable,
                str(pathlib.Path(maindir, "engine", "ci_helpers.py")),
                "autopublish", "--repo", maindir, "--diff-base", "library"],
...
def press_run(now_iso):
    ...
    return B.build(maindir, library_state(), str(sitedir),
...
findings, auto = night_shift_run("nb/n1-micron", "semiconductors", "micron",
                                 dossier, "2026-07-05")
check("dossier PR validates BLOCK-clean in CI mode",
      findings["block_count"] == 0, detail=str(findings["findings"]))
check("autopublish gate says true", auto == "true")
editor_merge("nb/n1-micron")

brief1 = make_fixtures.brief("2026-07-05")
pre_merge_ref = subprocess.run(["git", "rev-parse", "library"], cwd=root,
                               capture_output=True, text=True).stdout.strip()
findings, auto = night_shift_run("nb/n1-brief", "ai-briefs", "2026-07-05",
                                 brief1, "2026-07-05")
check("brief PR validates BLOCK-clean in CI mode",
      findings["block_count"] == 0, detail=str(findings["findings"]))
...
rival = brief1.replace("Daily brief for 2026-07-05",
                       "Rival brief for 2026-07-05")
findings, _ = night_shift_run("nb/n1-dupe", "ai-briefs", "2026-07-05",
                              rival, "2026-07-05", from_ref=pre_merge_ref)
check("already-published brief is BLOCKed (B-MODE)",
...
findings, auto = night_shift_run("nb/n2-brief", "ai-briefs", "2026-07-06",
                                 brief2, "2026-07-06")
check("night 2 brief validates BLOCK-clean",
      findings["block_count"] == 0, detail=str(findings["findings"]))
...
check("night 1 build page still exists",
      (sitedir / "builds" / "2026-07-05" / "index.html").is_file())
check("build pages link prev/next across nights",
      'href="../2026-07-05/"' in
      (sitedir / "builds" / "2026-07-06" / "index.html").read_text())
feed2 = ET.fromstring((sitedir / "feed.xml").read_text())
check("global feed updated to 3 entries",
      len(feed2.findall(f"{NS}entry")) == 3)
series_feed = ET.fromstring(
