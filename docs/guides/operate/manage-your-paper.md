...
"commission a deep dive on ASML", "less policy in the brief for a while",
"use a cheaper model for research", "give the paper a new look". Each lands
as one small diff under `press/`.
...
article's slug must be a configured item, and both article initialization and
CI enforce that.
