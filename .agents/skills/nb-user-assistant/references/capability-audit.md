...
Configuration is not proof. Run the canary below. A local simulation, a run in
the setup chat, or a different provider environment cannot substitute.

## Run the immediate one-series canary
...
ordinary due cadence for the canary, then restore `manual` after the test; a
manual series is correctly invisible to `nb duty`.

Use the scheduler's on-demand trigger with its final prompt, repository,
permissions, and runtime. Limit the canary to the chosen series when the
scheduler supports appending that scope without restating the production
workflow.
...
delivery. Do not merge merely to complete the audit; review the canary as the
first issue of the paper.

## Audit CI and publication

Confirm the canary used the `pull_request` validation workflow from the base
branch, the proof loaded engine and press state from `main`, and the render
probe targeted the candidate article. After the user approves and merges the
canary, confirm the Pages URL serves it and the catalog/feed update. CI and
Pages remain distinct checks even when GitHub hosts both.
...
Resume at: rerun the one-series canary from branch delivery
```

Give the user only the `Next action` when it requires them. After the external
state changes, repeat the narrowest reliable test, then resume the canary from
...
scheduled runtime produced the passing canary PR. Otherwise report `not ready`,
the failed or untested requirement, and the single next action.
