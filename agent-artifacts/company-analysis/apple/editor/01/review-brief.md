# Editor review-brief — company-analysis/apple (01)
...
- The **exact writer brief**: `agent-artifacts/company-analysis/apple/writer/01/brief.md`
  (read it so copied instructions / prompt leakage into the prose are detectable)
- The draft: `library/company-analysis/apple.html`
- Evidence: `agent-artifacts/company-analysis/apple/researcher/01/evidence.md`
- Voice guide: `agent-artifacts/company-analysis/apple/writing-coach/01/voice-guide.md`
...
- Return to **researcher**: a claim whose evidence is missing, wrong, or
  unverifiable.
- Recompute the segment/margin figures against Apple's filing. Confirm the tariff-refund impact is attributed correctly (Apple's own quantification vs press-derived). Confirm NO buy/sell/allocation call. Verify each chart's numbers against its committed provenance script and that captions cite the source.
## Prove it after your edits
Re-run: `/home/user/the-nightly-build/nb check library/company-analysis/apple.html --series company-analysis --repo /home/user/the-nightly-build` and confirm **BLOCK: 0** still holds.

## Output
Write `agent-artifacts/company-analysis/apple/editor/01/editorial-review.md`: the record
of the three reads, every direct edit you made, every source you re-checked and
its result, and your decision.

## Report
- If no required change remains: `DONE editor library/company-analysis/apple.html`
