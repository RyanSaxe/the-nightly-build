# Evidence record: build-from-scratch/flash-attention (researcher 02)

This is a complete record for the round-02 evidence request. Round 01's four
sources and findings are reproduced below unchanged; nothing in them was
contradicted or superseded. Three new sources are added, each opened and
confirmed, each tied to a claim the article already makes and currently leaves
uncited: Vaswani et al. for the scaled dot-product attention the piece
rebuilds, Jia, Maggioni, Staiger, and Scarpazza's Volta microbenchmarking
report for the measured HBM-is-slow / SRAM-is-fast hardware facts, and the
Dao-AILab/flash-attention repository for the shipped kernel the closing
section compares against. A fourth, optional candidate — a safe-softmax
numerical-stability reference predating Milakov and Gimelshein (2018) — was
searched for and not added; see Discarded for why.

The one finding that changes what the writer can safely do with the new
material: Vaswani et al.'s own attention formula is *scaled* dot-product
attention, Attention(Q,K,V) = softmax(QKᵀ/√d_k)V, with the 1/√d_k division
stated as load-bearing (it keeps large-d_k dot products out of the softmax's
flat-gradient region). The article's own code — `S = Q @ K.T`, no division —
implements the unscaled form, which matches FlashAttention's own Algorithm 0
exactly but not Vaswani's Equation 1 exactly. This is recorded in full under
Contradictions so the writer cites Vaswani for the query/key/value dot-product/
softmax/weighted-sum structure the piece rebuilds, not for a formula the
piece's own code does not implement.

## Sources

Entries 1-4 are preserved from researcher/01/evidence.md without change.
Entries 5-7 are new to this round.
...
3. **New this round: Vaswani et al.'s attention formula is scaled; the
   article's own formula, matching FlashAttention's Algorithm 0, is not.**
...
Figures 1-9 are preserved from round 01. Figures 10-11 are new.
...
Preserved from round 01, plus two new entries for the added sources.
...
```text
URL: Candidate not added — a safe-softmax numerical-stability reference
     predating Milakov and Gimelshein (2018), as the brief's optional fourth
     item. Searched specifically for a pre-2018 primary establishing the
     max-subtraction shift as a numerical-stability fix. Found nothing that
     clears the bar: the max-subtraction identity is treated in the
     literature as a folk numerical-computing technique with no single
     canonical originating paper, and general log-sum-exp stability write-ups
     found in the search either predate softmax's ML usage without discussing
     it directly, or postdate Milakov and Gimelshein (e.g., Blanchard,
     Higham, and Higham, "Accurately Computing the Log-Sum-Exp and Softmax
     Functions," IMA Journal of Numerical Analysis, 2021 — a genuine
     peer-reviewed primary for softmax numerical-stability error bounds, but
     published three years after Milakov and Gimelshein, so it does not
     satisfy "predating" and would not add a distinct claim-owner beyond what
     Milakov and Gimelshein (already cited, s1/s2 in this record) already
     establishes for the exact fix the article uses). Adding either would
     pad the list with a source that does not own a claim the article makes
     and Milakov/Gimelshein does not already cover. Not added.
```

## Report to orchestrator

Three sources added and confirmed open, each tied to a claim already in the
article and currently uncited: Vaswani et al. (arXiv:1706.03762) for the
query/key/value dot-product-then-softmax-then-weighted-sum structure the
piece rebuilds in its full-matrix-baseline section; Jia, Maggioni, Staiger,
and Scarpazza (arXiv:1804.06826) for the measured HBM-is-slow / SRAM-is-fast
hardware facts the orientation and closing sections assert; and the
Dao-AILab/flash-attention repository (github.com/Dao-AILab/flash-attention)
for the shipped kernel the closing section's "fused CUDA kernel" language
refers to. The optional fourth candidate, a safe-softmax reference predating
Milakov and Gimelshein, was searched for and not found to clear the bar; it is
not added, so this round brings the source count from four to seven, not
eight — the owner's floor is not fully met by legitimate sources on the
territory the editor named, and that gap should be reported rather than
padded closed.

The one item the writer needs before citing the new Vaswani source: it
defines *scaled* dot-product attention, with the 1/√d_k division load-bearing
in the paper's own account, while the article's own code (matching
FlashAttention's own Algorithm 0) is unscaled. Cite Vaswani for the shape of
the computation the piece rebuilds, not for the specific unscaled formula the
code implements — see Contradictions #3 for the exact phrasing risk.
