...
Checkout revision: `95c2d74961ce9cbad64272443ceb717e735b2d02`
...
Source: `templates/paper/identity.md`

# paper

A paper article reports on one research paper: what it claims, how the claim is
earned, and where the claim stops. Its subject is the paper, never the
experience of reading about it.

Open with the source. The `abstract` section carries the paper card (title,
authors, venue, link) and the abstract verbatim, so the original's own words are
on record before any commentary.

Rebuild the argument in your own words, your own examples, and the order that
teaches best. Do not assume the original's order does. Define each concept at
the moment the argument first spends it. Anchor the moments the argument turns
on in the citations themselves, with an honest `data-nb-locator` and
`data-nb-note` on the cite, and quote the original's exact sentence in a note
when it earns display space.

Weigh the evidence as a reviewer would: what was measured, on what, and what
would count as the result failing to generalize. Name a weak evaluation plainly.
The focal paper owns its claims. Another source earns space only when it changes
the interpretation. It never replaces the paper or pads the list. Place the
paper among the work it builds on and the work that disagrees with it, and state
a verdict on its claims before the sources.

## 6. Series direction

Source: `press/series/paper-of-the-day/prompt.md`

# Paper of the Day

Follow research in machine learning and artificial intelligence. Choose a paper
when rebuilding its central claim would clarify an active technical problem or
change how current work looks once the abstract is set aside. The paper should
give the article something to examine, not just a famous result to announce.

Prefer work with a public record after publication: later use, a replication, a
serious criticism, or follow-on research that lets the article weigh the claim
