# Evidence: paper-of-the-day/variational-autoencoder (researcher/03)

This is a complete record superseding researcher/02/evidence.md, not an
overwrite: all seven prior sources, their contradictions, numbers, and source
assets are carried forward unchanged below. The brief's first-choice
candidate, Rezende, Mohamed, and Wierstra 2014 (arXiv:1401.4082), is already
source 2 in this record — it was read in full in researcher/01 and is already
cited in the draft (the "developed independently... at almost the same
moment" passage in the reparameterization-trick section). Re-adding it would
not raise the count; it is reported here as a check performed, not a new
source. The brief's second candidate, Doersch 2016 ("Tutorial on Variational
Autoencoders," arXiv:1606.05908), was read in full this round and is added as
source 8. It resolves at its own arXiv abstract page and is a genuine
claim-owner, not padding: in Section 2.2, immediately after deriving the
reparameterized gradient (its own Eq. 10), Doersch states in his own voice —
not a restatement of Kingma and Welling's text, which never states this
generalization explicitly — the exact necessary conditions under which the
trick is valid: the noise source must be fixed and unlearned, the
reparameterizing function must be continuous in the input, and consequently
the trick cannot apply to a discrete latent distribution. That is precisely
the claim the article's gradient-problem and reparameterization-trick
sections rest on and that editor/01 pushed on directly — the review's own
note that "the trick works precisely because [the latents] are continuous"
was previously asserted by the article without a citation owning it. Doersch
is the author making that generalization himself, so for this specific claim
he is primary, not merely a secondary explainer of Kingma and Welling. The
one gap that remains unchanged from researcher/02: Higgins et al. 2017
(beta-VAE), source 5, is still not a primary read; this round did not retry
it, since the brief states it is genuinely unreachable in this session and
directs effort at the eighth source instead. The total now stands at eight
sources: seven carried forward unchanged, one newly read and load-bearing
(source 8 below), one still access-limited and flagged rather than silently
dropped.
...
Kind:        primary in principle — this is beta-VAE's only official home;
             ...
             STILL NOT UPGRADED. This round did not retry it: the brief
             states the primary is genuinely unreachable in this session and
             directs the round at a different source instead, so retrying
             the same exhausted routes would not be a good use of this
             round. Round 2's finding stands unchanged: OpenReview gates
             this specific paper behind an interactive Cloudflare Turnstile
             challenge that requires solving a widget in a real browser,
             which no tool available in this session can do. That gap, and
             every route tried against it in researcher/01 and researcher/02,
             is recorded in full in the source 5 entry and in Discarded
             below, carried forward unchanged.
...
Kind:        primary — Ho, Jain, and Abbeel's own paper; owns the DDPM
             ...
             bound. Added researcher/02 for the ELBO-to-diffusion forward
             link.
...
Kind:        primary — Paisley, Blei, and Jordan's own paper; owns the
             ...
             Welling cite by name but do not derive. Added researcher/02
...
Kind:        primary, for the specific claim below — Carl Doersch's own
             tutorial. Added this round as the eighth source. The document
             as a whole is expository (it restates, in its own words, the
             VAE derivation Kingma and Welling own), but the passage cited
             here is Doersch's own analytical generalization, stated in his
             own voice and not present in this explicit form in either
             Kingma and Welling 1312.6114 or Rezende et al. 1401.4082, so
             for this claim he is the author who owns it, not a secondary
             reporting someone else's result. Confirmed the URL resolves to
             the paper's own arXiv abstract page (HTTP 200; title, author,
             and abstract match the PDF). Read the PDF directly (23 pages,
             extracted with pdfplumber) through Section 2.4, not a summary.
Establishes: The general necessary conditions under which the
             reparameterization trick is valid at all, stated once, in
             general form, immediately after the paper derives its own
             version of the reparameterized gradient (its Eq. 10, the same
             identity as the focal paper's Eq. 4-6 under different
             notation): the auxiliary noise variable must come from a fixed,
             not-learned distribution, and the reparameterizing function
             must be continuous in the conditioning variable x so that
             backpropagation can pass through it. Doersch draws the
             consequence out explicitly and names it as a limitation the
             focal paper's own text leaves implicit: because the function
             must be continuous in x, the technique cannot be applied when
             the latent distribution itself is discrete. This is the
             specific claim the article's gradient-problem and
             reparameterization-trick sections depend on and that editor/01
             pushed on directly, repairing a sentence that had mis-signalled
             discreteness ("no path through a discrete draw" to "no
             differentiable path through the random draw") without yet
             citing the source that states, in general terms, why continuity
             is the load-bearing requirement and discreteness is exactly
             what breaks it.
Paraphrase:  Title "Tutorial on Variational Autoencoders," author Carl
             Doersch (Carnegie Mellon / UC Berkeley). The PDF's own header
             gives "August 16, 2016, with very minor revisions on January 3,
             2021"; arXiv's own submission history lists v1 19 June 2016, v2
             13 August 2016, v3 3 January 2021 — the same paper, the header
             date matching the v2 revision closest to it. Section 2.2
             ("Optimizing the objective") first derives the reparameterized
             gradient for the paper's own running Gaussian case (its
             Eq. 10, sampling z = mu(X) + Sigma^{1/2}(X)*eta with
             eta ~ N(0,I), the same substitution as the focal paper's Eq. 9
             sampling line under different symbols), illustrated in the
             paper's Figure 4 (two versions of the same feedforward network,
             with and without the trick, non-differentiable sampling
             operations marked in red). Immediately after, in the same
             section, the paper generalizes beyond the Gaussian case to
             state the necessary conditions in general form, which is the
             passage cited here.
Locators:    Section 2.2 "Optimizing the objective," the paragraph
             immediately following Eq. (10), PDF page 11 of 23 (the page
             directly preceding the start of Section 2.3 "Testing the
             learned model").
Quote:       "It's worth pointing out that the 'reparameterization trick'
             only works if we can sample from Q(z|X) by evaluating a
             function h(eta,X), where eta is noise from a distribution that
             is not learned. Furthermore, h must be continuous in X so that
             we can backprop through it. This means Q(z|X) (and therefore
             P(z)) can't be a discrete distribution! If Q is discrete, then
             for a fixed eta, either h needs to ignore X, or there needs to
             be some point at which h(eta,X) 'jumps' from one possible value
             in Q's sample space to another, i.e., a discontinuity."
```
...
unchanged; researcher/02's two additions and this round's addition do not
contradict the focal paper — they extend and independently corroborate its
machinery (Paisley, Blei, and Jordan own the variance claim the focal paper
cites; Ho, Jain, and Abbeel generalize the same bound rather than dispute it;
Doersch generalizes the trick's own necessary conditions rather than
disputing anything the focal paper claims).
...
No new figures this round. Doersch's tutorial is cited for a general,
qualitative necessary-condition claim, not a number.
...
```text
Asset: None found for a "reparameterization schematic" in the focal paper
       itself.
Shows: I read every figure in the focal paper (Figures 1 through 5, main
       text and both appendices) and none of them diagrams the
       reparameterization trick. Figure 1 (page 2) is the graphical-model
       diagram (solid lines for the generative model, dashed for the
       variational approximation) — a candidate for illustrating the
       latent-variable model itself, not the reparameterization. Figures 2
       and 3 (Section 5) are lower-bound and marginal-likelihood convergence
       plots. Figure 5 (Appendix A) is a grid of random MNIST samples at
       varying latent dimensionality. The reparameterization trick is
       presented only as prose and equations (Section 2.4, page 4-5).
Crop:  N/A.
```
...
```text
Asset: Doersch's Figure 4, "A training-time variational autoencoder
       implemented as a feed-forward neural network" — Section 2.2, page 10.
       Two side-by-side network diagrams, without and with the
       reparameterization trick; red marks the non-differentiable sampling
       operation, blue marks loss layers.
Shows: The same substitution the focal paper's Eq. 9-10 makes, drawn as a
       network diagram rather than written as arithmetic: sampling z
       directly leaves no path back to mu and sigma (left panel, red
       sampling node sits between the parameters and the loss), while
       sampling eps first and building z by elementwise arithmetic keeps mu
       and sigma inside the differentiable graph (right panel).
Crop:  Not recommended as an added asset. The article already carries an
       original nb-code contrast that makes exactly this point in code
       (naive `Normal(mu, sigma).sample()` versus `mu + sigma * eps`), so
       this figure would duplicate rather than add evidence. Recorded for
       completeness, not as a recommendation to use it.
```
...
URL: https://openreview.net/pdf?id=Sy2fzU9gl — same access failure as the
     ...
     Not retried this round per the brief's direction.
```
...
URL: http://web.archive.org/web/20260430152217/https://openreview.net/pdf?id=Sy2fzU9gl
     ...
     snapshot exists from 2026-04-30, but fetching that snapshot failed on
     every attempt across researcher/01 and researcher/02: the plain-HTTP
     form was blocked by egress policy outright, and the HTTPS form reset
     the TLS connection before completing the handshake's response. Not
     rejected on merit — the snapshot may well contain the readable PDF —
     rejected because this session cannot reach web.archive.org, a
     different and separate failure from OpenReview's own Turnstile gate.
```
...
URL: https://api.semanticscholar.org/graph/v1/paper/... (both the search
     ...
     every attempt in researcher/02, including after two delays of 15-20
     seconds. Could not confirm or read any open-access PDF field it might
     carry.
```
...
URL: https://arxiv.org/abs/1503.03585 (Sohl-Dickstein, Weiss, Maheswaranathan,
     ...
     Thermodynamics," 2015) — read the abstract and opening of the PDF in
     researcher/02 to confirm identity: this is the paper that originates
     the diffusion probabilistic model Ho et al. 2020 builds on and cites as
     [53], and its own framing is explicitly variational ("a parameterized
     Markov chain trained using variational inference," per Ho et al.'s own
     description of it). Not added as a separate source entry: the brief
     names Sohl-Dickstein 2015 "and/or" Ho et al. 2020, and Ho et al. 2020
     alone supplies the explicit "usual variational bound" sentence, its own
     equation number, and a direct, quotable locator. Citing both for the
     same single claim would be the padding the brief warns against.
```

## Reported to the orchestrator

The brief's preferred candidate, Rezende, Mohamed, and Wierstra 2014, was
checked against the existing record before doing anything else: it is
already source 2, read in full in researcher/01 and already cited in the
draft for the independent-derivation claim, so it could not supply the
eighth source without padding the count with a repeat. The brief's second
candidate, Doersch 2016, was read in full this round (Section 2.2 in
particular, through the paragraph immediately after its Eq. 10) and is
added as source 8. It resolves cleanly at its own arXiv abstract page
(confirmed HTTP 200; title, author, and abstract verified against the PDF)
and genuinely owns a claim the article depends on: the general necessary
conditions for the reparameterization trick to work at all — a fixed,
unlearned noise source and a function continuous in the conditioning
variable, with the explicit consequence that a discrete latent distribution
breaks the trick. That is the exact claim underlying the wording editor/01
repaired in the gradient-problem section ("no differentiable path through
the random draw," changed from language that mis-signalled discreteness),
and it was previously asserted without a citation that owns it. Higgins et
al. 2017 (beta-VAE), source 5, remains the one source not read as a primary;
this round did not retry it, since the brief states the access limitation is
already established and genuine, and directs the round at the eighth source
instead. The total source count is now eight: seven carried forward
unchanged from researcher/02, one newly read and load-bearing (source 8
above), one still access-limited and clearly flagged rather than silently
dropped or falsely marked as upgraded. This reaches the series floor of
eight with a source that changes what the reader can verify, not one added
to hit a number.
