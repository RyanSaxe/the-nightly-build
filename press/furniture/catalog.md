...
The components this paper adds to the engine's base catalogue
(`templates/FURNITURE.md`), styled in `styles.css` beside this file on the `rs-`
prefix (`nb-` is the engine's). Any section may reach for them,
so they live here, not in one template's folder. A section that uses one
