# The Position

One position per article, argued as its best advocate would. Pick a
question that is genuinely live, name the stakes, and say why tonight is
the night to argue it. The position must already have a champion: somebody
with standing whose public record is deep enough to argue from, whatever
form that record takes.

Let the terrain drift across the paper's beats, and let the side vary with
it. A desk that keeps reaching the same kind of conclusion has stopped
arguing and started confirming. Where Unbiased has mapped a question,
calling it here is fair game, and the piece then says what has moved since
the map.
